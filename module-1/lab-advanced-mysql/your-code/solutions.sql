...
SELECT v.Author_ID AS 'Author ID', SUM(v.Suma) AS 'TOTAL PROFITS'
	FROM(SELECT t.title_id AS 'Titel_ID', t.au_id AS 'Author_ID', SUM(t.Sales) AS 'Suma'
	FROM(SELECT	titles.title_id, authors.au_id, titles.price * sales.qty * titles.royalty / 100 * titleauthor.royaltyper / 100 AS 'Sales'
			FROM titles
			JOIN titleauthor
			ON titles.title_id = titleauthor.title_id
...
SELECT t.title_id AS 'Titel_ID', t.au_id AS 'Author_ID', SUM(t.Sales) AS 'Suma'
	FROM(SELECT	titles.title_id, authors.au_id, titles.price * sales.qty * titles.royalty / 100 * titleauthor.royaltyper / 100 AS 'Sales'
			FROM titles
			JOIN titleauthor
			ON titles.title_id = titleauthor.title_id
...
SELECT	au.au_id AS 'AUTHOR ID', SUM(t.price * s.qty * t.royalty / 100 * ta.royaltyper / 100) AS 'TOTAL PROFITS'
		FROM titles t
		JOIN titleauthor ta
		ON t.title_id = ta.title_id
		JOIN authors au
...
CREATE TABLE most_profiting_authors SELECT	au.au_id AS 'AUTHOR ID', SUM(t.price * s.qty * t.royalty / 100 * ta.royaltyper / 100) AS 'TOTAL PROFITS'
		FROM titles t
		JOIN titleauthor ta
		ON t.title_id = ta.title_id
		JOIN authors au
